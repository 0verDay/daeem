...
##     见 view/troop_grid.gd 的 MODE_UNITS）。
##
...
func set_troop(troop, leader_name_text: String = "") -> void:
	...
	visible = _troop.get("leader", null) != null
	queue_redraw()
...
## 格子第一行写的字（= 将领真名；拿不到名字时退回外面递进来的占位文案）
func leader_name() -> String:
	return label_text()


## 格子第二行小字：「x/y」（x = 现有单位数，y = 编制上限）
...
func _draw_leader_cell(f: Font) -> void:
	...
	_draw_centered(f, short_name(l), av, UiStyleRes.TEXT, UiStyleRes.FS_BODY)
	...
	draw_string(f, Vector2(UiLayoutRes.ROSTER_CELL_TEXT_X, UiLayoutRes.ROSTER_ID_Y),
		_clip_text(f, label_text(), UiLayoutRes.ROSTER_ID_W, UiStyleRes.FS_SMALL),
