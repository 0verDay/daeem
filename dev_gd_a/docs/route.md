...
| 命令卡的内容 | **随右侧页签实时切换**，而页签是**按选中对象动态显示**的（第二十二节）：操作页 = 移动(Q)/攻击(W)/行军(E)/停止(A)，单位页 = 占位单位(Q)，建筑页 = 城墙(Q)/箭塔(W)，招募页 = 三个占位将领，科技页空着 | 单位 / 招募两张表都是数据驱动的（`config.recruit.list`、`config.recruit.zone.list`），建筑页来自 `building.DEFS` 的 `buildable` |
...
| `page_tabs.gd` | 右下那一列**动态页签**（`set_pages()`：选中部队 = 操作/单位，区划中心 = 招募，大本营 = 科技，普通建筑 = 一颗都没有，什么都没选中 = 建筑；见第二十二节） |
...
| 什么都没选中 | **建筑** | 城墙(Q) / 箭塔(W)（建造入口住在这儿，手玩定的） |
