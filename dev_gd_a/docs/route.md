...
| 详细信息 | (400, 840)，**1030×240**（参考图直接标了） | 第三轮改版后 = **左右两栏**（左 = 当前展开的部队 + 将领头像网格；右 = 选中单位头像 / 名称 / buff / 数值）—— 见[第十七节](#十七详细信息栏改版左右两栏--将领头像网格--buff-占位这一轮) |
...
| `unit_roster.gd` / `troop_grid.gd` | ★ 详细信息左栏的上半（当前展开的部队）/ 下半（将领头像网格）—— **第三轮改版新增**，见第十七节 |
| `detail_panel.gd` | 底栏详细信息（第三轮改版：**左右两栏** —— 左 = `unit_roster` + `troop_grid`，右 = 选中单位 + 招募五格 + 提示行；见第十七节） |
...
| `tests/test_ui.gd` `_test_box_select` | 框到亲兵 = 整支部队、多支部队一起选中、敌人不算、空框清空、Shift 追加、真实事件路径、部队 UI 高亮、方块列表分组 / 编号 / 方块字 / 几何、`_draw` 真的跑过 |


## 十七、详细信息栏改版：左右两栏 / 将领头像网格 / buff 占位（这一轮）
